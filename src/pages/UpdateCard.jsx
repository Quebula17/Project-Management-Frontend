...
/>
            ...
            <Button type="submit" variant="contained" color="primary" fullWidth style={{ backgroundColor: '#3f51b5' }} sx={{ fontFamily: 'Poppins'}}>Update</Button>
        </Box>
    );
};
...
const UpdateCard = () => {
    ...
    const getCardData = async () => {
        ...
        let response = await api.get(`/api/cards/${cardId}`, {
            Authorization: `Bearer ${JSON.parse(localStorage.getItem('access_token'))}`
        })
        if (response.status === 200){
            setCard({
                ...
                task_list: response.data.task_list.list_id,
    ...
    const handleSubmit = async (event) => {
        ...
        let response = await api.put(`/api/cards/${cardId}/`, 
        JSON.stringify(card),
        {
            headers: {
                'Authorization': `Bearer ${JSON.parse(localStorage.getItem('access_token'))}`,
